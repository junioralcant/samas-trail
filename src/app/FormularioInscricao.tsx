...
type FormularioInscricaoProps = {
  ...
  disponivelCamisa: Record<string, number>;
};
...
export default function FormularioInscricao({
  ...
  const [camisas, setCamisas] = useState<Selecao>({});
  const [disponivelCamisas, setDisponivelCamisas] = useState(disponivelCamisa);
  ...
  const qtdCamisas = totalSelecionado(camisas);
  const totalCamisas = qtdCamisas * precoCamisa.precoAtual;
  const camisasEsgotadas = Object.values(disponivelCamisas).every(
    (n) => n === 0,
  );
  const total = preco - desconto + totalCamisas;
    ...
    try {
      const response = await fetch("/api/inscricoes", {
        ...
        body: JSON.stringify({
          ...
          termoAceito,
          camisasExtras: paraItens(camisas),
        }),
      });
      const data = await response.json();
      // Camisa esgotada durante o preenchimento não pode derrubar a
      // inscrição: corrige a seleção e o atleta segue.
      if (response.status === 409 && Array.isArray(data.faltas)) {
        const novoDisponivel = { ...disponivelCamisas };
        const novaSelecao = { ...camisas };
        for (const falta of data.faltas as {
          tamanho: string;
          disponivel: number;
        }[]) {
        ...
        setDisponivelCamisas(novoDisponivel);
        setCamisas(novaSelecao);
        setErro(data.erro ?? "Um tamanho de camisa acabou enquanto você se inscrevia.");
        return;
      }
      if (!response.ok) {
  ...
  return (
    <main>
      ...
      {/* Peça paga, ao lado de um kit que diz "incluso": a separação é
          feita por cor, etiqueta, nome e preço, todos visíveis de uma vez. */}
      <section className="secao-camisa-extra">
        <div className="camisa-extra-cabecalho">
          <div className="kit-titulo-linha">
            <div className="kit-barra" />
            <h2 className="titulo-secao display">Camisa extra</h2>
            <span className="pill-a-venda">À venda</span>
          </div>
          <div className="camisa-extra-separacao">{SEPARACAO_KIT.home}</div>
        </div>

        <div className="camisa-extra-corpo">
          <div className="camisa-extra-galeria">
            <GaleriaCamisa />
          </div>
          <div className="camisa-extra-acao">
            <SeloPromocao preco={precoCamisa} />
            {camisasEsgotadas ? (
              <div className="camisa-extra-esgotada">Camisas esgotadas</div>
            ) : (
              <>
                <Link className="botao-cta camisa-extra-cta" href="/camisa">
                  Comprar camisa
                </Link>
                <div className="camisa-extra-nota">
                  Também dá para adicionar durante a inscrição, no mesmo
                  pagamento.
                </div>
              </>
            )}
      ...
      <div className="conteudo">
        ...
        <form className="coluna" onSubmit={enviarInscricao}>
          ...
          {/* Entre o cupom e o termo — nunca encostado no aviso de menor
              de idade, que ocupa a faixa logo acima do termo. */}
          <div className="bloco-camisa-extra">
            <div className="bloco-camisa-topo">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                className="bloco-camisa-mini"
                src="/camisa-extra/frente.jpeg"
                alt="Camisa extra azul-royal"
              />
              <div className="bloco-camisa-texto">
                <div className="bloco-camisa-nome">Adicionar camisa extra</div>
                <div className="bloco-camisa-separacao">
                  {SEPARACAO_KIT.formulario}
                </div>
              </div>
              <SeloPromocao preco={precoCamisa} tamanho="medio" />
            </div>

            {camisasEsgotadas ? (
              // O bloco colapsa em aviso: sem seletor, sem CTA morto, e a
              // inscrição segue normalmente.
              <div className="bloco-camisa-esgotado">
                As camisas extras acabaram — eram 54 peças, sem reposição. Sua
                inscrição continua normalmente, e a camiseta do kit segue
                inclusa.
              </div>
            ) : (
              <SeletorCamisas
                disponivel={disponivelCamisas}
                selecao={camisas}
                aoMudar={setCamisas}
              />
            )}
          ...
          {/* Discriminado: a camisa só ganha linha quando há camisa, e o
              cupom diz em que ele desconta. */}
          <div className="resumo-total">
            <div className="resumo-linha">
              <span>
                Inscrição · {distancia} · {loteAtual}
              </span>
            ...
            {qtdCamisas > 0 && (
              <div className="resumo-linha">
                <span>
                  Camisa extra · {qtdCamisas} ×{" "}
                  {formatarPreco(precoCamisa.precoAtual)}
                </span>
                <span>{formatarPreco(totalCamisas)}</span>
              </div>
            )}
            {cupomAplicado && (
              <div className="resumo-linha resumo-linha-desconto">
                <span>Cupom {cupomAplicado.codigo} · inscrição</span>
            ...
            {cupomAplicado && qtdCamisas > 0 && (
              <div className="resumo-nota">
                O cupom vale na inscrição. A camisa já está com o preço
                promocional.
              </div>
            )}
          </div>
